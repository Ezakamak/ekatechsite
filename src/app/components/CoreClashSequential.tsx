...
function LobbyHome({ c, user, busy, open, mine, createLobby, joinLobby, enterLobby }: { c: any; user: User | null; busy: boolean; open: Lobby[]; mine: Lobby[]; createLobby: (event: FormEvent<HTMLFormElement>) => void; joinLobby: (id: number) => void; enterLobby: (id: number) => void }) {
  return <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]"><div className="space-y-6"><form onSubmit={createLobby} className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-xl"><div className="flex items-center gap-3"><div className="rounded-2xl border border-purple-300/20 bg-purple-300/10 p-3 text-purple-100"><UserPlus className="h-5 w-5" /></div><div><h2 className="text-2xl font-medium">{c.create}</h2><p className="mt-1 text-sm text-white/45">{c.both}</p></div></div><button disabled={busy || !user} className="mt-6 w-full rounded-full bg-white px-5 py-3 text-sm font-semibold text-black disabled:opacity-50">{busy ? "..." : c.create}</button>{!user && <p className="mt-3 text-sm text-red-100/70">Sign in gerekli.</p>}</form><LobbyList title={c.mine} items={mine} c={c} empty={c.empty} action={(lobby) => enterLobby(lobby.id)} actionText={c.enter} /></div><LobbyList title={c.open} items={open} c={c} empty={c.empty} action={(lobby) => joinLobby(lobby.id)} actionText={c.join} /></section>;
}
function LobbyList({ title, items, c, empty, action, actionText }: { title: string; items: Lobby[]; c: any; empty: string; action: (lobby: Lobby) => void; actionText: string }) { return <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 backdrop-blur-xl"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-cyan-100" /><h2 className="text-2xl font-medium">{title}</h2></div><div className="mt-5 space-y-3">{items.length ? items.map((lobby) => { const map = mapFor(lobby.map_key); return <div key={lobby.id} className="rounded-3xl border border-white/10 bg-black/30 p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Avatar player={{ name: lobby.creator_name, email: lobby.creator_email, avatar_url: lobby.creator_avatar_url }} /><div><p className="text-xs uppercase tracking-[0.14em] text-white/30">{c.host}</p><p className="font-medium text-white">{lobby.creator_name || lobby.creator_email}</p></div></div><span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/55">{map.name}</span></div><div className="mt-4 flex items-center justify-between gap-3"><p className="text-sm text-white/45">{lobby.opponent_user_id ? c.live : c.waiting}</p><button onClick={() => action(lobby)} className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-black">{actionText}</button></div></div>; }) : <p className="rounded-3xl border border-white/10 bg-black/30 p-5 text-sm text-white/45">{empty}</p>}</div></div>; }
