...
function TradeCard({ trade, currentUserId, inventoryItems, tr, locale, copy, onAccept, onDecline, onCancel, onUpdateItems, onReady, onConfirm }: { trade: TradeOffer; currentUserId: number; inventoryItems: InventoryItem[]; tr: boolean; locale: string; copy: Record<string, string>; onAccept: () => void; onDecline: () => void; onCancel: () => void; onUpdateItems: (itemId: string, quantity: number) => void; onReady: (ready: boolean) => void; onConfirm: () => void }) {
  ...
  const [quantity, setQuantity] = useState(String(Number(isProposer ? trade.offer_quantity : trade.request_quantity || 1)));
  const parsedQuantity = Number(quantity);
  const validQuantity = Number.isFinite(parsedQuantity) && parsedQuantity > 0 ? Math.floor(parsedQuantity) : 0;
  ...
  return <div className="rounded-[1.5rem] border border-white/10 bg-black/30 p-4">
    ...
    {canEdit && <div className="mt-4 grid gap-3 rounded-2xl border border-white/10 bg-white/[0.035] p-3 sm:grid-cols-[1fr_96px_auto]">
      ...
      <label className="grid gap-2 text-xs text-white/55">{copy.qty}<input type="number" min={1} step="any" value={quantity} onChange={(event) => setQuantity(event.target.value)} className="rounded-2xl border border-white/10 bg-black/60 px-3 py-2 text-white outline-none" /></label>
      <button onClick={() => itemId && validQuantity > 0 && onUpdateItems(itemId, validQuantity)} disabled={!itemId || validQuantity <= 0} className="self-end rounded-full bg-white px-4 py-2 text-xs font-semibold text-black hover:bg-gray-200 disabled:opacity-40">{copy.saveItem}</button>
    </div>}
