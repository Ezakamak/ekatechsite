...
export function MarketAcademy() {
  ...
  const missions = useMemo(() => {
    ...
    return [
      ...
      { done: concentrationOk, title: copy.concentration, desc: copy.concentrationDesc },
  ...
  function handleTrade(side: "buy" | "sell") {
    ...
    if (qty < 1) return setTemporaryMessage(copy.invalidQty);
    ...
    <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-28 pt-28 text-white sm:px-6">
      ...
      <div className="relative mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <LessonCard icon={<Building2 />} title={copy.lessonSqlTitle} text={copy.lessonSqlText} />
        </section>
      </div>
    </main>
  );
...
function normalizeMarketState(data: any): MarketState {
  ...
  return {
    ...
    news,
    rewards,
    techCoin: { balance: safeNumber(techCoin?.balance), lifetime_earned: safeNumber(techCoin?.lifetime_earned) },
  };
}
...
function InfoCell({ label, value }: { label: string; value: ReactNode }) {
  return <div><p className="text-white/35">{label}</p><div className="mt-1 font-medium">{value}</div></div>;
}

function InfoBox({ label, value }: { label: string; value: ReactNode }) {
  return <div className="rounded-2xl border border-white/10 bg-black/25 p-4"><p className="text-xs uppercase tracking-[0.16em] text-white/35">{label}</p><div className="mt-2 text-xl font-semibold text-white">{value}</div></div>;
}

function InfoLine({ label, value }: { label: string; value: ReactNode }) {
  return <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-black/25 px-4 py-3"><span className="text-white/40">{label}</span><span className="font-medium text-white">{value}</span></div>;
}
...
function NewsCard({ title, empty, lesson, dayLabel, news, tr, locale }: { title: string; empty: string; lesson: string; dayLabel: string; news: NewsItem[]; tr: boolean; locale: string }) {
  return <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl sm:p-6"><p className="text-sm uppercase tracking-[0.2em] text-white/35">{title}</p><div className="mt-5 space-y-3">{news.length === 0 ? <p className="rounded-3xl border border-white/10 bg-black/25 p-5 text-sm leading-6 text-white/50">{empty}</p> : news.map((item, index) => <NewsItemCard key={`${item.day}-${item.target}-${index}`} item={item} lesson={lesson} dayLabel={dayLabel} tr={tr} locale={locale} />)}</div></div>;
}

function NewsItemCard({ item, lesson, dayLabel, tr, locale }: { item: NewsItem; lesson: string; dayLabel: string; tr: boolean; locale: string }) {
  return <div className="rounded-3xl border border-white/10 bg-black/25 p-4"><div className="flex items-center gap-2 text-xs text-white/35"><Newspaper className="h-4 w-4" /> {dayLabel} {item.day} · {item.target}</div><p className="mt-2 font-medium">{tr ? item.titleTr : item.titleEn}</p><p className={`mt-2 text-sm ${item.tone === "positive" ? "text-emerald-300" : item.tone === "negative" ? "text-red-300" : "text-cyan-300"}`}>{item.impact > 0 ? "+" : ""}{formatPrice(item.impact * 100, locale)}%</p><p className="mt-3 text-sm leading-6 text-white/45"><span className="text-white/70">{lesson}:</span> {tr ? item.lessonTr : item.lessonEn}</p></div>;
}

function LessonCard({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl sm:p-6"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-purple-300/20 bg-purple-300/10 text-purple-100">{icon}</div><h3 className="mt-4 text-xl font-medium">{title}</h3><p className="mt-3 text-sm leading-6 text-white/45">{text}</p></div>;
}

const trCopy = {
  eyebrow: "Tech Coin cüzdanına bağlı eğitim borsası",
  title: "EkaTrade Academy",
  subtitle: "EkaTrade OFF Tech Coin cüzdanındaki bakiye ile çalışır. Alımda cüzdan bakiyesi azalır, satışta cüzdana geri eklenir.",
  ...
  overconcentratedText: "Portföyün tek hisseye veya tek sektöre fazla yığılmış. Tek kötü haber portföyü sert etkileyebilir.",
  highRisk: "Yüksek risk",
  highRiskText: "Dağılım hâlâ zayıf. En az 3 farklı hisse ve mümkünse farklı sektörler kullan.",
  balanced: "Dengeli",
  balancedText: "Portföy tek noktaya yığılmamış. Bu, temel risk yönetimi açısından daha sağlıklı.",
  ...
  lessonPriceTitle: "Fiyat = beklenti",
  lessonPriceText: "Hisse fiyatı sadece bugünkü durumu değil, geleceğe dair beklentiyi de taşır.",
  lessonRiskTitle: "Risk bakiyeyi etkiler",
  lessonRiskText: "Bu sürümde al/sat OFF Tech Coin cüzdanını etkiler; amaç gereksiz işlem değil kontrollü öğrenmedir.",
  lessonSqlTitle: "SQL mantığı",
  lessonSqlText: "Cüzdan, portföy, fiyat geçmişi ve haberler ayrı tablolarda tutulur.",
  detailLearningText: "Büyük grafik yönü gösterir ama tek başına karar sebebi değildir; haber, sektör ve portföy ağırlığı birlikte okunur.",
};

const enCopy = {
  ...trCopy,
  eyebrow: "Tech Coin wallet market academy",
  subtitle: "EkaTrade uses the OFF Tech Coin wallet balance. Buying subtracts from the wallet; selling adds back to it.",
