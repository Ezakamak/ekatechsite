...
export function TechDuelSync({ initialLobbyId }: { initialLobbyId?: number | null }) {
  ...
  const ownedOpenLobbyIds = useMemo(() => [...openLobbies, ...myLobbies].filter((lobby) => lobby.status === "open" && user?.id === lobby.creator_user_id).map((lobby) => lobby.id), [openLobbies, myLobbies, user?.id]);
  useAutoCloseOpenLobbies("tech_duel", ownedOpenLobbyIds);
  ...
  const closeLobby = async (id: number) => {
    setBusy(true); setNotice(null);
    try {
      await closeOpenLobby("tech_duel", id);
      setNotice({ type: "success", text: c.closed });
      await loadDuels(true);
    } catch (e) { setNotice({ type: "error", text: e instanceof Error ? e.message : "Lobby kapatılamadı." }); }
    finally { setBusy(false); }
  };
