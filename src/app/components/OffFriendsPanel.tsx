...
export function OffFriendsPanel() {
  ...
  const loadOnline = async ({ silent = false }: { silent?: boolean } = {}) => {
    try {
      if (!silent) setLoadingOnline(true);
      setOnlineError(null);
      await pingPresence();
      const res = await fetch("/api/off/users/online");
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "Could not load online users");
      setOnlineUsers(data.users || []);
    } catch (_error: any) {
      setOnlineError("Could not load online users");
      if (!silent) toast.error("Could not load online users");
    } finally {
      if (!silent) setLoadingOnline(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      void loadOnline({ silent: true });
    }, ONLINE_REFRESH_MS);
    return () => window.clearInterval(id);
  }, []);

  const sendRequest = async (userId: number) => { const res = await fetch("/api/off/friends/request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId }) }); const data = await res.json().catch(() => ({})); if (!res.ok) return toast.error(data.error || "Hata"); toast.success("İstek gönderildi"); loadCore(); loadOnline({ silent: true }); };
  const respond = async (friendshipId: number, action: "accept"|"reject") => { const res = await fetch("/api/off/friends/respond", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ friendshipId, action }) }); const data = await res.json(); if (!res.ok) return toast.error(data.error || "Hata"); toast.success(action === "accept" ? "İstek kabul edildi" : "İstek reddedildi"); loadCore(); loadOnline({ silent: true }); };
  const removeFriend = async (userId: number) => { const res = await fetch("/api/off/friends/remove", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId }) }); if (res.ok) { toast.success("Arkadaş silindi"); loadCore(); loadOnline({ silent: true }); } else toast.error("Hata"); };
  ...
  return <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl sm:p-6 text-white space-y-5">
    ...
    <div>
      ...
      {onlineError ? <p className="text-sm text-rose-200/90">Could not load online users</p> : onlineUsers.length === 0 ? <p className="text-sm text-white/60">No online users right now</p> : <div className="grid gap-2 md:grid-cols-2">{onlineUsers.map((u)=> userRow(u, <button onClick={()=>sendRequest(u.id)} className={`${btn} border-cyan-300/40 bg-gradient-to-r from-cyan-400/25 to-purple-500/25 text-cyan-100`}>Arkadaş ekle</button>))}</div>}
    </div>
