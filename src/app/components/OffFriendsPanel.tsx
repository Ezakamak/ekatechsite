...
export function OffFriendsPanel() {
  ...
  const saveProfile = async () => { const res = await fetch("/api/off/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(profile) }); if (!res.ok) return toast.error("Profil kaydedilemedi"); toast.success("OFF profil güncellendi"); load(); };

  const userRow = (u: Item, action?: ReactNode) => <div key={`${u.id}-${u.friendshipId || "x"}`} className="rounded-2xl border border-white/15 bg-white/[0.05] px-3 py-2 flex items-center justify-between gap-3"><div className="flex items-center gap-3 min-w-0"><div className="h-9 w-9 shrink-0 rounded-full bg-gradient-to-br from-cyan-500/40 to-purple-500/40" /> <div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{u.displayName}</p><p className="text-xs text-white/65">Lvl {u.level || 1}{u.selectedTitle ? ` · ${u.selectedTitle}` : ""}</p></div></div>{action}</div>;
  ...
  return <section className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl sm:p-6 text-white space-y-5">
    ...
    <div><p className="text-sm text-white/70 mb-2">Gelen İstekler</p>{incoming.map((r)=> userRow(r, <div className="space-x-2"><button onClick={()=>respond(Number(r.friendshipId||r.id),"accept")} className={`${btn} border-emerald-300/40 bg-emerald-400/20 text-emerald-100`}>Kabul et</button><button onClick={()=>respond(Number(r.friendshipId||r.id),"reject")} className={`${btn} border-rose-300/40 bg-rose-500/10 text-rose-100`}>Reddet</button></div>))}</div>
