...
const HIDDEN_FROM_ACTIVE_ADMIN_LIST = ["completed", "rejected"];
  ...
  try {
    const requests = await context.env.DB
      .prepare(`
        ...
        WHERE
          project_requests.status NOT IN ('completed', 'rejected')
    ...
    return Response.json({ error: "Proje talepleri alınamadı. priority, target_date ve project_feedback alanlarını kontrol et." }, { status: 500 });
  }
}
...
export async function onRequestPatch(context: any) {
  const admin = await requireAdminOrOwner(context);
  if (!admin.ok) return Response.json({ error: admin.error }, { status: admin.status });
  if (!canMutateAdminData(admin.user)) return Response.json({ error: "Sipariş yönetmek için onaylı admin profili gerekli." }, { status: 403 });

  try {
    const body = await context.request.json().catch(() => null);
    const requestId = Number(body?.requestId);
    const status = String(body?.status || "");
    const allowed = ["received", "reviewing", "offer_ready", "waiting_approval", "development_started", "revision", "delivered", "completed", "rejected", "new", "reviewed", "contacted", "accepted"];

    if (!requestId || !allowed.includes(status)) return Response.json({ error: "Geçersiz talep veya durum." }, { status: 400 });
    ...
    if (!current) return Response.json({ error: "Proje talebi bulunamadı." }, { status: 404 });
    ...
    const shouldRelease = ["received", "new"].includes(status);
    const shouldHideFromAdminList = HIDDEN_FROM_ACTIVE_ADMIN_LIST.includes(status);
    const nextAssignedAdminId = shouldRelease || shouldHideFromAdminList ? null : current.assigned_admin_id || admin.user.id;
    ...
    if (updateResult?.meta?.changes === 0) return Response.json({ error: "Proje artık bu admin hesabına atanmış değil." }, { status: 409 });
    ...
    await writeAuditLog(context, {
      ...
      details: `${admin.user.name} proje durumunu ${current.status} → ${status} yaptı.${shouldHideFromAdminList ? " Proje aktif admin listesinden kaldırıldı." : nextAssignedAdminId ? " Proje admine atandı." : " Proje tüm adminlere açıldı."}`,
    });
    ...
    return Response.json({
      success: true,
      message: shouldHideFromAdminList
        ? "Talep güncellendi ve aktif admin listesinden kaldırıldı."
        : nextAssignedAdminId
          ? "Talep güncellendi ve sorumlu admine atandı."
