...
export async function onRequestGet(context: any) {
  ...
  try {
    ...
    const comments = await context.env.DB.prepare(
      `
        SELECT
          ...
          COALESCE(user_levels.level, 1) AS user_level
    ...
    return json(
      { error: "Yorumlar alınamadı.", detail: readableError(error) },
      500,
    );
  }
}
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    if (!symbol) return json({ error: "Hisse sembolü gerekli." }, 400);
    if (comment.length < 3)
      return json({ error: "Yorum en az 3 karakter olmalı." }, 400);
    if (comment.length > 420)
      return json({ error: "Yorum 420 karakteri geçemez." }, 400);

    const stock = await context.env.DB.prepare(
      `SELECT symbol FROM market_stocks WHERE symbol = ?`,
    )
      .bind(symbol)
      .first();
    ...
    await context.env.DB.prepare(
      `
        INSERT INTO market_stock_comments (symbol, user_id, comment, status, approved_at)
    ...
    return json({
      ...
      message: level.verified
        ? "Mavi tikli yorumun direkt yayımlandı."
        : "Yorumun direkt yayımlandı.",
    });
  } catch (error) {
    return json(
      { error: "Yorum gönderilemedi.", detail: readableError(error) },
      500,
    );
  }
}
...
async function ensureMarketComments(context: any) {
  await context.env.DB.prepare(
    `CREATE TABLE IF NOT EXISTS user_levels (user_id INTEGER PRIMARY KEY, exp INTEGER NOT NULL DEFAULT 0, level INTEGER NOT NULL DEFAULT 1, updated_at TEXT DEFAULT CURRENT_TIMESTAMP)`,
  ).run();
  await context.env.DB.prepare(
    `CREATE TABLE IF NOT EXISTS market_stock_comments (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT NOT NULL, user_id INTEGER NOT NULL, comment TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending', reviewed_by INTEGER, reviewed_at TEXT, approved_at TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP)`,
  ).run();
  await context.env.DB.prepare(
    `CREATE INDEX IF NOT EXISTS idx_market_stock_comments_symbol_status ON market_stock_comments(symbol, status)`,
  ).run();
  await context.env.DB.prepare(
    `CREATE INDEX IF NOT EXISTS idx_market_stock_comments_status_created ON market_stock_comments(status, created_at)`,
  ).run();
}
