...
export async function onRequestPost(context: any) {
  ...
  try {
    await ensureChatTables(context);
    const limitResult = await checkSimpleLimit(
      context,
      `roulette-chat:${auth.user.id}`,
      12,
      60,
    );
    if (!limitResult.ok)
      return Response.json(
        { error: "Çok hızlı mesaj gönderiyorsun. Biraz bekle." },
        { status: 429 },
      );

    await context.env.DB.prepare(
      `INSERT INTO tech_roulette_chat_messages (user_id, message, created_at) VALUES (?, ?, datetime('now'))`,
    )
...
async function ensureChatTables(context: any) {
  ...
  await context.env.DB.prepare(
    `CREATE INDEX IF NOT EXISTS idx_tech_roulette_chat_created ON tech_roulette_chat_messages(created_at DESC)`,
  ).run();
}

async function checkSimpleLimit(
  context: any,
  key: string,
  limit: number,
