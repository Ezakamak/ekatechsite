...
export async function onRequestGet(context: any) {
  ...
  try {
    ...
    return json({ error: "Eka Towers verisi alınamadı.", detail: readableError(error) }, { status: 500 });
  }
}
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    if (action === "start") return json(await startRound(context, auth.user.id, body));
    if (action === "reveal") return json(await revealTile(context, auth.user.id, body));
    if (action === "cashout") return json(await cashoutRound(context, auth.user.id));
...
async function startRound(context: any, userId: number, body: any) {
  ...
  const requestedDifficulty = String(body?.difficultyKey || "medium").toLowerCase();
  const difficultyKey: DifficultyKey = isDifficultyKey(requestedDifficulty) ? requestedDifficulty : "medium";
  ...
  await context.env.DB.prepare(`INSERT INTO coin_transactions (user_id, amount, reason) VALUES (?, ?, ?)`).bind(userId, -betAmount, "Eka Towers bet").run();
  ...
  await context.env.DB
    ...
    `)
    .bind(userId, betAmount, difficultyKey, JSON.stringify(createMatrix(difficultyKey)))
    .run();
...
async function revealTile(context: any, userId: number, body: any) {
  ...
  if (!Number.isFinite(tileIndex) || tileIndex < 0 || tileIndex >= TILES_PER_ROW) throw new Error("Geçersiz kutucuk.");
  ...
  if (revealed.some((item) => item.level === level && item.tileIndex === tileIndex)) return { ...(await buildState(context, userId, round)), message: "already_revealed" };
  ...
  if (tile === "trap") {
    await context.env.DB
      .prepare(`UPDATE techcoin_towers_rounds SET revealed_json = ?, status = 'lost', ended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
      .bind(JSON.stringify(nextRevealed), round.id)
      .run();
    return { ...(await buildState(context, userId, { ...round, revealed_json: JSON.stringify(nextRevealed), status: "lost" })), message: "trap_hit" };
  }
  ...
  if (nextLevel >= LEVELS) {
    const payout = payoutForLevel(Number(round.bet_amount), LEVELS, round.difficulty_key);
    await creditWallet(context, userId, payout, "Eka Towers perfect cashout");
    await context.env.DB
      .prepare(`UPDATE techcoin_towers_rounds SET revealed_json = ?, current_level = ?, current_multiplier = ?, payout_amount = ?, status = 'won', ended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
      .bind(JSON.stringify(nextRevealed), LEVELS, multiplier, payout, round.id)
      .run();
    return { ...(await buildState(context, userId, { ...round, revealed_json: JSON.stringify(nextRevealed), current_level: LEVELS, current_multiplier: multiplier, payout_amount: payout, status: "won" })), message: "tower_cleared", payout };
  }

  await context.env.DB
    .prepare(`UPDATE techcoin_towers_rounds SET revealed_json = ?, current_level = ?, current_multiplier = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    .bind(JSON.stringify(nextRevealed), nextLevel, multiplier, round.id)
    .run();

  return { ...(await buildState(context, userId, { ...round, revealed_json: JSON.stringify(nextRevealed), current_level: nextLevel, current_multiplier: multiplier })), message: "safe" };
}
...
async function cashoutRound(context: any, userId: number) {
  ...
  if (clearedLevels <= 0) throw new Error("Cashout için önce en az 1 seviyeyi geçmelisin.");

  const payout = payoutForLevel(Number(round.bet_amount), clearedLevels, round.difficulty_key);
  await creditWallet(context, userId, payout, "Eka Towers cashout");
  await context.env.DB
    .prepare(`UPDATE techcoin_towers_rounds SET payout_amount = ?, status = 'cashed', ended_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    .bind(payout, round.id)
    .run();

  return { ...(await buildState(context, userId, { ...round, payout_amount: payout, status: "cashed" })), message: "cashed", payout };
}
...
function serializeRound(round: ActiveRound) {
  ...
  const difficultyKey = isDifficultyKey(String(round.difficulty_key)) ? round.difficulty_key : "medium";
  ...
  return {
    ...
    matrix: matrix.map((row, level) => row.map((kind, tileIndex) => ({
      id: `${round.id}-${level}-${tileIndex}`,
      kind: showAll || revealed.some((item) => item.level === level && item.tileIndex === tileIndex) ? kind : null,
      revealed: showAll || revealed.some((item) => item.level === level && item.tileIndex === tileIndex),
    }))),
  };
}
...
function payoutForLevel(betAmount: number, level: number, difficultyKey: DifficultyKey) {
  return Math.max(0, Math.floor(Number(betAmount || 0) * multiplierForLevel(level, difficultyKey)));
}
...
function parseMatrix(value: string): TileKind[][] {
  try {
    ...
    return Array.from({ length: LEVELS }, (_, level) => {
      ...
      return Array.from({ length: TILES_PER_ROW }, (_, tileIndex) => row[tileIndex] === "trap" ? "trap" : "safe");
    });
  } catch {
...
function parseRevealed(value: string) {
  try {
    ...
    return parsed
      .map((item) => ({ level: Number(item?.level), tileIndex: Number(item?.tileIndex) }))
      .filter((item) => Number.isInteger(item.level) && item.level >= 0 && item.level < LEVELS && Number.isInteger(item.tileIndex) && item.tileIndex >= 0 && item.tileIndex < TILES_PER_ROW);
  } catch {
    return [];
  }
}
...
async function getActiveRound(context: any, userId: number): Promise<ActiveRound | null> {
  return await context.env.DB
    .prepare(`SELECT * FROM techcoin_towers_rounds WHERE user_id = ? AND status = 'active' ORDER BY id DESC LIMIT 1`)
    .bind(userId)
    .first();
}
...
async function ensureTowersTables(context: any) {
  ...
  await context.env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_techcoin_towers_rounds_active_user ON techcoin_towers_rounds(user_id, status)`).run();
}
