...
export async function onRequestGet(context: any) {
  ...
  const row = await context.env.DB.prepare(`
    SELECT u.id, u.name, u.username, u.display_name, op.display_name AS off_display_name,
           op.avatar_url, op.banner_url, op.bio, op.selected_title, op.selected_badge,
  ...
  return Response.json({ ok: true, profile: {
    userId: Number(row.id), displayName: resolveDisplayName(row), avatarUrl: row.avatar_url || null,
    bannerUrl: row.banner_url || null, bio: row.bio || null, selectedTitle: row.selected_title || null,
