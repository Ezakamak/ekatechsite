...
export async function onRequestGet(context: any) {
  ...
  const lastRun = await context.env.DB.prepare("SELECT created_at FROM off_cleanup_runs ORDER BY id DESC LIMIT 1").first<any>();
  const shouldRun = !lastRun?.created_at || (Date.now() - Date.parse(String(lastRun.created_at).replace(' ', 'T') + 'Z')) > 5 * 60 * 1000;
  if (shouldRun) await runOffCleanup(context, { triggerType: 'opportunistic', dryRun: false });
  const rows = await context.env.DB.prepare(`SELECT i.*, inviter.name inviter_name, inviter.email inviter_email, inviter.nickname inviter_nickname, invp.display_name inviter_off_display_name, invitee.name invitee_name, invitee.email invitee_email, invitee.nickname invitee_nickname, inteep.display_name invitee_off_display_name FROM off_game_invites i JOIN users inviter ON inviter.id=i.inviter_id JOIN users invitee ON invitee.id=i.invitee_id LEFT JOIN off_profiles invp ON invp.user_id=inviter.id LEFT JOIN off_profiles inteep ON inteep.user_id=invitee.id WHERE ((i.status='pending' AND i.expires_at > datetime('now') AND (i.inviter_id=? OR i.invitee_id=?)) OR (i.inviter_id=? AND i.status IN ('accepted','rejected','expired') AND i.responded_at >= datetime('now', '-5 minutes'))) ORDER BY i.id DESC LIMIT 120`).bind(auth.user.id, auth.user.id, auth.user.id).all<any>();
  const normalized = (rows?.results || []).map((row: any) => { const s = (()=>{try{return JSON.parse(row.game_settings_json||"{}")}catch{return {}}})(); const lobbyId = row.lobby_id ? Number(row.lobby_id) : null; return ({ inviteId: Number(row.id), inviter: { id: Number(row.inviter_id), displayName: resolveDisplayName({ id: row.inviter_id, name: row.inviter_name, email: row.inviter_email, nickname: row.inviter_nickname, off_display_name: row.inviter_off_display_name }) }, invitee: { id: Number(row.invitee_id), displayName: resolveDisplayName({ id: row.invitee_id, name: row.invitee_name, email: row.invitee_email, nickname: row.invitee_nickname, off_display_name: row.invitee_off_display_name }) }, gameKey: row.game_key, gameLabel: INVITABLE_GAMES[row.game_key]?.label || row.game_key, gameMode: row.game_mode || row.duel_mode || null, modeName: toModeName(String(row.game_mode || row.duel_mode || "")), roundCount: Number(row.round_count || 5), mapKey: s.mapKey || null, mapName: s.mapKey ? toMapName(s.mapKey) : null, status: row.status, expiresAt: row.expires_at, createdAt: row.created_at, respondedAt: row.responded_at || null, lobbyId, redirectTo: row.status === 'accepted' && lobbyId ? routeFor(row.game_key, lobbyId) : null, targetRoute: row.target_route || null }); });
  return Response.json({ ok: true, incoming: normalized.filter((i: any) => i.invitee.id === auth.user.id && i.status === 'pending'), outgoing: normalized.filter((i: any) => i.inviter.id === auth.user.id && i.status === 'pending'), outgoingRecent: normalized.filter((i: any) => i.inviter.id === auth.user.id && ['accepted','rejected','expired'].includes(i.status)).map((i: any) => ({ inviteId: i.inviteId, status: i.status, gameKey: i.gameKey, gameLabel: i.gameLabel, lobbyId: i.lobbyId, redirectTo: i.redirectTo, respondedAt: i.respondedAt })) });
}
...
export async function onRequestPost(context: any) {
  ...
  if (gameKey === "tech_duel") {
    if (!gameCfg.modes.includes(gameMode) || !gameCfg.rounds.includes(roundCount)) return Response.json({ error: "Geçersiz Tech Duel ayarı." }, { status: 400 });
    selectedMode = gameMode;
  }
  if (gameKey === "core_clash") {
    const fallbackMap = "firewall_city";
    mapKey = gameCfg.maps.includes(mapKeyInput) ? mapKeyInput : (gameCfg.maps.includes(fallbackMap) ? fallbackMap : gameCfg.maps[Math.floor(Math.random() * gameCfg.maps.length)]);
  }

  const normalizedLegacyDuelMode =
    gameKey === "tech_duel" ? selectedMode || "classic" : "classic";

  const normalizedGameMode =
    gameKey === "tech_duel" ? selectedMode || "classic" :
    gameKey === "cipher_break" ? "standard" :
    gameKey === "core_clash" ? "standard" :
    "standard";

  const normalizedRoundCount =
    gameKey === "tech_duel" ? roundCount :
    gameKey === "cipher_break" ? 5 :
    gameKey === "core_clash" ? 1 :
    5;

  const normalizedSettings =
    gameKey === "tech_duel" ? { gameMode: normalizedGameMode, roundCount: normalizedRoundCount } :
    gameKey === "cipher_break" ? { roundCount: 5 } :
    gameKey === "core_clash" ? { mapKey } :
    {};

  const settingsJson = JSON.stringify(normalizedSettings);

  const dup = await context.env.DB.prepare(`SELECT id FROM off_game_invites WHERE inviter_id=? AND invitee_id=? AND game_key=? AND COALESCE(game_mode,duel_mode,'')=? AND COALESCE(game_settings_json,'{}')=? AND status='pending' AND expires_at>datetime('now') LIMIT 1`).bind(auth.user.id, friendId, gameKey, normalizedGameMode, settingsJson).first();
  if (dup) return Response.json({ error: "Bu oyun/ayar için aktif davet var." }, { status: 409 });

  const created = await context.env.DB.prepare(`INSERT INTO off_game_invites (inviter_id, invitee_id, game_key, duel_mode, game_mode, round_count, game_settings_json, target_route, status, message, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, '/off', 'pending', ?, datetime('now', '+5 minutes'))`).bind(auth.user.id, friendId, gameKey, normalizedLegacyDuelMode, normalizedGameMode, normalizedRoundCount, settingsJson, message || null).run();
  ...
  try {
    notificationResult = await createNotification(context, {
      ...
      body: gameKey === "tech_duel" ? `${actor} seni ${toModeName(normalizedGameMode)} modunda Tech Duel oynamaya davet etti.` : gameKey === "cipher_break" ? `${actor} seni Cipher Break maçına davet etti.` : `${actor} seni ${toMapName(String(mapKey || "firewall_city"))} haritasında Core Clash maçına davet etti.`,
      link: "/off",
  ...
  return Response.json({ ok: true, inviteId, inviteeId: friendId, gameKey, gameLabel: gameCfg.label, notificationCreated, notificationSkipped, notificationId: notificationResult?.id || null, notificationError });
}
