...
export async function onRequestGet(context: any) {
  ...
  const row = await context.env.DB.prepare(`SELECT u.id, u.name, u.username, u.display_name, op.* FROM users u LEFT JOIN off_profiles op ON op.user_id=u.id WHERE u.id=?`).bind(uid).first<any>();
...
export const onRequestPatch = async (context: any) => {
  ...
  const payload = {
    display_name: clean(body.display_name, 24), bio: clean(body.bio, 160), avatar_url: clean(body.avatar_url, 512), banner_url: clean(body.banner_url, 512),
    selected_title: clean(body.selected_title, 64), selected_badge: clean(body.selected_badge, 64)
  };
